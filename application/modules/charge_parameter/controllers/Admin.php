...
class Admin extends Auth_controller
{
	...
	public function __construct()
	{
		...
		$this->table = 'charge_parameter';
		$this->title = 'Charge Parameter';
		$this->redirect = 'charge_parameter';
	}
	...
	public function form($id = '')
	{
		...
		$data['detail'] = $detail;
		if (isset($detail->charge_code)) {
			$data['charge_code'] = $detail->charge_code;
		} else {
			$last_row_no = $this->crud_model->get_where_single_order_by('charge_parameter', array('status' => '1'), 'id', 'DESC');
			if (isset($last_row_no->charge_code)) {
				// $string = "IS07042022-0006";
				$string = $last_row_no->charge_code;
				$explode = explode("-", $string);
				$int_value = intval($explode[1]) + 1;
				// var_dump(sprintf("%04d", $int_value));
				$data['charge_code'] = 'CHCD' . date('dmY') . '-' . sprintf("%04d", $int_value);
			} else {
				$data['charge_code'] = 'CHCD' . date('dmY') . '-0001';
			}
		}
		if ($this->input->post()) {
			$this->form_validation->set_rules('charge_name', 'Charge Name', 'required|trim');
