	...
	public function form($id = '')
	{
		...
		if (isset($detail->transfer_code)) {
			...
			if (isset($last_row_no->transfer_code)) {
				...
				$data['transfer_code'] = 'LocT' . date('dmY') . '-' . sprintf("%04d", $int_value);
			} else {
				$data['transfer_code'] = 'LocT' . date('dmY') . '-0001';
			}
		}
		...
		if ($this->input->post()) {
			...
			// exit;
			$this->form_validation->set_rules('transfer_code', 'Scrap Code', 'required|trim');
		...
		$data['location_details'] = $this->crud_model->get_where('location_para', array('status' => '1'));
		// echo "<pre>";
		// var_dump($data['location_details']);
		// exit;
